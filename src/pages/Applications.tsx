...
const Applications = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (isTeacher) {
          promises.push(apiRepository.getReceivedCollaborationSubmissions());
        }
        ...
        setProjects(res[2]);
        setGroups(res[3]);
        setParticipants(res[4]);
        setGroupMembers(res[5]);
        
        if (isTeacher && res[6]) {
          setExternalSubmissions(res[6]);
        }
      } catch (e) {
  ...
  return (
    <div className="container py-8 max-w-6xl mx-auto px-4">
      ...
      {/* Tabs Selector for Teachers */}
      {isTeacher && (
        <div className="flex gap-4 border-b border-border/60 mb-8 overflow-x-auto scrollbar-none">
          <button
            onClick={() => setActiveTab('registered')}
            className={cn(
              "pb-4 text-sm font-bold relative transition-all duration-300 flex items-center gap-2 px-1 focus:outline-none whitespace-nowrap",
              activeTab === 'registered' ? "text-primary scale-[1.02]" : "text-muted-foreground hover:text-foreground"
            )}
          >
            <UserCircle2 className="h-4 w-4" />
            Registered Students ({displayApps.length})
          ...
          <button
            ...
            className={cn(
              "pb-4 text-sm font-bold relative transition-all duration-300 flex items-center gap-2 px-1 focus:outline-none whitespace-nowrap",
              activeTab === 'external' ? "text-primary scale-[1.02]" : "text-muted-foreground hover:text-foreground"
            )}
          >
            <Globe className="h-4 w-4" />
            External Collaboration ({filteredExternalSubmissions.length})
            {activeTab === 'external' && (
              <motion.div layoutId="activeTabIndicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-full" />
            )}
          </button>
