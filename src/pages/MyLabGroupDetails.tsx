  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Authorize by checking lab-admin access for this group's lab.
        await apiRepository.getLabAdmins(g.lab_id);
        ...
        console.error('MyLabGroupDetails load error:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container py-10 max-w-5xl">
      <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
        <Link to={`/my-labs/labs/${group.lab_id}`} className="inline-flex items-center text-sm text-muted-foreground hover:text-primary mb-6 transition-colors gap-1.5">
          <ArrowLeft className="h-4 w-4" /> Back to My Lab
        </Link>
      </motion.div>
...
export default MyLabGroupDetails;
